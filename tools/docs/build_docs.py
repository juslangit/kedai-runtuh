...
NAME = "Kedai Runtuh"
ARTIFACT = "https://claude.ai/artifact/DEHnMjnwaX4awbdwZCyLFF"   # the private link the --artifact copy is published to
...
GALLERIES = [
    ("screens-run", "A run, from the hanging tray to the fall",
     "Moments from several runs, in the order a run goes. The camera rises with the tower, and the "
     "swing gets faster with every piece that stays on.", "tall", [
        ("docs/shots/01_first_hang.png",
    ...
    ("screens-menus", "Menus, settings and the icon",
...
TOOLS = [
    ...
    ("GDScript", "All the game: the loop and food list, the pendulum, the rope, the UI theme, the save file and the app icon, all drawn in code."),
